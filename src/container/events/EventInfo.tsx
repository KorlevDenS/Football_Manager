...
import {InputAdornment, FormHelperText, TextField} from "@mui/material";
...
export default function EventInfo({event, handleClose, getEvent, setLoggedIn}: EventInfoProps) {
    ...
    useEffect(() => {
        setCollectiveEventChange(event);
        if (event.type === "Матч")
            getMatch().then();
    ...
    const updateCollectiveEvent = async () => {
        ...
        await fetch("/event/update/collective/event", {
            ...
            body: JSON.stringify([event, collectiveEventChange])
        })
            .then(response => {
                    if (response.ok) {
                        getEvent();
