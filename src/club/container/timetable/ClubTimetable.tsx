import '../../../container/timetable/Timetable.css'
...
import ClubEventContainer from "../../events/ClubEventContainer";
import ClubEventLayout from "../../events/ClubEventLayout";
...
export default function CLubTimetable({club, setLoggedIn}: ClubTimeTableProps) {
    const [weekOffset, setWeekOffset] = useState(0);
    const [events, setEvents] = useState<ClubCollectiveEvent[]>([]);
    const [eventsSchedule, setEventsSchedule] = useState<Array<Array<ClubCollectiveEvent>>>([]);
    const [week, setWeek] = useState<Date[]>(calcWeek(weekOffset));
    const navigate = useNavigate();
    const [showingEvent, setShowingEvent] = useState<ClubCollectiveEvent>();
    ...
    const formWeekSchedule = (events: ClubCollectiveEvent[]) => {
        let schedule: ClubCollectiveEvent[][] = [];
        for (let i = 0; i < 7; i++) {
            let arr: ClubCollectiveEvent[] = [];

            for (let j = 0; j < events.length; j++) {
                if (new Date(events[j].collectiveEvent.date).getDay() === i) arr.push(events[j]);
            }
            schedule.push(arr);
        }
                    ...
                    if (response.ok) {
                        ...
                        data.then(value => {eventsArray = value as ClubCollectiveEvent[]})
                            .then(() => setEvents(eventsArray
                                .sort((a, b) =>
                                    new Date(a.collectiveEvent.date).getTime() - new Date(b.collectiveEvent.date).getTime())
    ...
    const loadEventInfo = (event: ClubCollectiveEvent) => {
        let e = event;
    ...
    const handleClose = (updateView: boolean) => {
        navigate("/club/manager/Timetable");
        ...
        <div className='Timetable'>
            <Routes>
                ...
                <Route path={"event/about/*"} element={
                    <>
                        {showingEvent != null && (
                            <ClubEventContainer event={showingEvent} getEvent={loadTimetable} setLoggedIn={setLoggedIn}
...
interface ClubDaysLayoutProps {
    day: Date;
    dayEvents: Array<ClubCollectiveEvent>;
    loadEventInfo(event: ClubCollectiveEvent): void;
}

function DaysLayout({day, dayEvents, loadEventInfo}: ClubDaysLayoutProps) {
